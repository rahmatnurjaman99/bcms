...
<div class="space-y-8">
	<div>
		...
		<p class="text-sm text-muted-foreground">Monitor team access and recent activity.</p>
	</div>
		...
		{#if userSummary}
			<Card>
				<CardHeader>
					...
					<CardDescription>Active accounts synced with {APP_NAME}.</CardDescription>
				</CardHeader>
				<CardContent>
					<p class="text-3xl font-semibold">{userSummary.total}</p>
					<p class="text-sm text-muted-foreground">Latest {userSummary.recent.length} shown below.</p>
				</CardContent>
			</Card>
		{/if}
		...
		{#if roles}
			<Card>
				...
				<CardContent>
					...
					<p class="text-sm text-muted-foreground">Synced with Spatie Permission.</p>
				</CardContent>
			</Card>
		{/if}
		...
		{#if permissions}
			<Card>
				<CardHeader>
					...
					<CardDescription>Granular capabilities across products.</CardDescription>
				</CardHeader>
				<CardContent>
					<p class="text-3xl font-semibold">{permissions.length}</p>
					<p class="text-sm text-muted-foreground">Managed centrally.</p>
				</CardContent>
			</Card>
		{/if}
	</div>
	...
	{#if userSummary}
		<Card>
			<CardHeader>
				...
				<CardDescription>Latest accounts created in the last sync window.</CardDescription>
			</CardHeader>
	...
	{:else}
		<Card>
			<CardHeader>
				...
				<CardDescription>Insufficient permission to view user stats.</CardDescription>
			</CardHeader>
		</Card>
	{/if}
</div>
